...
% Mention that decimal and hexadecimal encoding are analogous to binary encoding
% and they just use other "values" \cite{Katoch2021}


\subsubsection{Value Encoding}
\cite{Katoch2021}
% similar to binary encoding, but the values can be real nubers, integers
% or characters
...
\subsubsection{\enquote{Matrix Encoding}}
% In quotes, because it's not an actual encoding. It is more what the encoding
% represents - namely a matrix.
% depending on the meaning of the encoded data, an encoded matrix could
% be interpreted as permutation encoding or
% Matrices can be used to encode the adjacency matrix/representation of
% ordering problems \cite{Affenzeller2009}.
